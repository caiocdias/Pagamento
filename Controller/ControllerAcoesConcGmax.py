...
class ControllerAcoesConcGmax:
    ...
    def gerar_producao_por_supervisor(self, lista_supervisores: list):
        """
        Gera a relação por SUPERVISOR:
        - Para cada supervisor, cria (se necessário) uma subpasta com o período
          <YYYY-MM-DD_a_YYYY-MM-DD> dentro da pasta do supervisor.
        - Exporta um PDF por pessoa do supervisor dentro dessa subpasta.
        - Exporta um único Excel por supervisor (com uma aba por pessoa) dentro dessa subpasta.
        """
        try:
            cols_soma = ["ACOES_QTD_US_INTERNA", "ACOES_QTD_US_TOP", "ACOES_QTD_US_PRJ", "ACOES_QTD_US_GEO"]
            ...
            return f"Erro ao exportar relação por supervisor. {str(e)}"
